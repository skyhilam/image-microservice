...
class S3FileStream
{
     ...
     */
    public function __construct($request, string $filePath, string $adapter = 's3')
    {
        $this->request    = $request;
        $this->filePath   = $filePath;
        $this->filesystem = app('cloudfile')->getDriver();
        $this->adapter    = app('cloudfile')->getAdapter();
    ...
    protected function setHeaders()
    {
        ...
        if (!is_null($this->request->server('HTTP_RANGE'))) {
            ...
            [$_, $range] = explode('=', $this->request->server('HTTP_RANGE'), 2);
    ...
    protected function stream()
    {
        ...
        $video = new \Symfony\Component\HttpFoundation\StreamedResponse(
